#This is the code for computing the question:
#Let k be an integer such that the triangle with vertices (k,-3k),(5,k),(-k,2) has area 28. Find the orthocentre of this triangle.   
...
#for solving the quadratic equation obtained from Determinant
def Solve_Quadratic(A):
	...
	if d >= 0:
		...
		return 0.01   #as the solution is of integer type we can return any number other than integer for the case of complex solution

#for calculating the directional vector along a line
def dir_vec(AB):
	return np.matmul(AB,dvec)

#for calculating the normal vector to the line
def norm_vec(AB):
	return np.matmul(omat,np.matmul(AB,dvec))

#for finding the point of intersection of lines
...
#for finding the foot of perpendicular from vertex to other side
...
print "The equations are", Det - 56, "and", Det +56
...
for i in range(0,l):
	...
	if X == 0 :
		k_value = k[i]
		print "The integral value of k satisfying the equations is", k_value
		
A = np.array([k_value,-3*k_value])
B = np.array([5,k_value])
C = np.array([-1*k_value,2])

print "The vertices of triangle are", A ,",",B,"and",C
...
print "The foot of perpendiculars from vertices of triangle are", P ,",",Q,"and",R
...
print "The orthocentre of the triangle is", H1
